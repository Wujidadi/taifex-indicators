...
<rules>
- 指定日期本身的結算日判斷：該日為當月第 3 個星期三，即 `weekday() == 2` 且日期落在 15～21 日之間。
- 指定日期為**星期四**時，額外判斷前一交易日是否為結算日（透過腳本輸出的 `前日結算日:` 欄位）：
  - 若 `前日結算日: TRUE` 且 `data.tsv` 中前一天的「是否結算日」為 `FALSE`，則將其更新為 `TRUE`。
...
2. 腳本會自行完成以下流程：
   ...
   - 呼叫 `src/finmind_mtx_daily_summary.py` 子程序取得摘要，解析其 stdout 所有欄位。
   - 以第 3 個星期三規則判斷指定日期本身是否為結算日。
   - 產生 7 欄 Tab 分隔的資料列，寫入（新增或更新）`data.tsv`。
   - 若指定日期為星期四且腳本輸出 `前日結算日: TRUE`，則同步更新 `data.tsv` 中前一天的結算標記。
   - 輸出操作結果（新增 / 更新 / 結算標記更新）。
