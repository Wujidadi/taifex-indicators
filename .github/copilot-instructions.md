...
主流程（多商品）分兩步，可透過 `/update-analysis-report` 技能一次完成，或直接呼叫腳本：
...
| `data/<id>.tsv`                | 各商品原始 OHLCV 資料，`<id>` 為商品代碼小寫（如 `data/mtx.tsv`、`data/tx.tsv`）                                             |
...
- `data/<id>.tsv`：各商品每日最近月 OHLCV 資料，依商品分檔\
  `data/tx.tsv` 為台指期（TX），自 1998 年 7 月 21 日台指期開市起至 2001 年 4 月 6 日（星期五）止\
  `data/mtx.tsv` 為小台指（MTX），自 2001 年 4 月 9 日（星期一）小台指開市起\
  舊版根目錄 `data.tsv` 則為 TX 接續 MTX 的單一序列，因此其 2001 年 4 月 6 日與 4 月 9 日兩個交易日的成交量有明顯斷層（6445 / 518），為正常現象
...
- `update-analysis-report`：
  - 位置：`.claude/skills/update-analysis-report/SKILL.md`
